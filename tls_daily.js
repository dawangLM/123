...
var cookie = $.getdata("tls_daily_ck")
message = ""
...
!(async () => {
    $.log(cookie);
    for ( var type of ["AddInteraction", "GetUserInfo", "ClickSign", "GetLaunchReward", "PlantGrassSeed", "TakeMilk", "Getanswer"] ) {
      switch(type) {
          case "AddInteraction":
            for ( var task of ["susuMeijia", "susuRiguangyu", "susuHuli"] ) {
              await tls(type, task);
            }
          default:
            await tls(type);
        ...
        $.post(
            ...
            async (err, resp, data) => {
                try {
                    ...
                    message += "\n"
                    message += data
                    const { errcode, errmsg, result: { nickname = "", signcount = "", grass_seed = "", milk = "", answerlist = [] } } = JSON.parse(data);
                    if (errcode != 0) {
                        $.log(errmsg);
                        resolve();
                    }
                    if (nickname.length > 0) {
                        $.log(`\n\n\n💪💪💪${nickname}已签到${signcount}天, 拥有${grass_seed}颗牧草种子和${milk}份奶滴\n\n\n`)
                    }
